...
export class CheckboxContentControl extends XmlComponent {
    ...
    private validateOptions(options: ICheckboxContentControlOptions): void {
        ...
        // Warn if children array is not empty
        if (options.children.length > 0) {
            console.warn(
                "CheckboxContentControl: Checkbox controls typically don't use child elements. " +
                    "The checkbox state is represented by symbol elements, not TextRun children. " +
                    "Consider using an empty children array: { children: [] }",
            );
        }
    }
