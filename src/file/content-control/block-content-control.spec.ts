...
describe("BlockContentControl", () => {
    ...
    describe("#constructor()", () => {
        ...
        it("should warn about spaces in tag", () => {
            ...
            new BlockContentControl({
                tag: "Customer Address", // Tag with spaces should trigger warning
                children: [new Paragraph("Test")],
            });

            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("Tag 'Customer Address' contains spaces"));
    ...
    describe("Enhanced Properties", () => {
        ...
        it("should handle all enhanced properties for block control", () => {
            ...
            expect(sdtPr.find((el: any) => el["w:richText"])).to.exist;

            // Verify specific values
            expect(sdtPr.find((el: any) => el["w:appearance"])["w:appearance"]._attr["w:val"]).to.equal("boundingBox");
            expect(sdtPr.find((el: any) => el["w:color"])["w:color"]._attr["w:val"]).to.equal("FF6600");
        });
        ...
        it("should maintain rich text marker for enhanced block controls", () => {
            ...
            expect(sdtPr.find((el: any) => el["w:text"])).to.not.exist;
        });
        ...
        it("should handle all Phase 2A properties for block controls", () => {
            ...
            expect(sdtPr.find((el: any) => el["w:text"])).to.not.exist;
            expect(sdtPr.find((el: any) => el["w:rPr"])).to.not.exist;
        });
    });
});
