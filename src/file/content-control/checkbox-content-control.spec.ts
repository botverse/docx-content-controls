...
describe("CheckboxContentControl", () => {
    ...
    describe("#constructor()", () => {
        ...
        it("should warn about non-empty children", () => {
            const consoleSpy = vi.spyOn(console, "warn").mockImplementation(() => {});

            new CheckboxContentControl({
                tag: "ChildrenWarning",
                children: [{ type: "unexpected" }], // Non-empty children should trigger warning
            });

            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("Checkbox controls typically don't use child elements"));
    ...
    describe("#prepForXml()", () => {
        it("should generate correct OOXML structure", () => {
            ...
            const checkboxElement = sdtPr.find((el: any) => el["w14:checkbox"]);
            expect(checkboxElement).to.exist;

            const checkboxContent = checkboxElement["w14:checkbox"];
            expect(checkboxContent.find((el: any) => el["w14:checked"])["w14:checked"]._attr["w14:val"]).to.equal("1");
        });
        ...
        it("should generate correct checked state", () => {
            ...
            const checkedElement = checkedSdtPr.find((el: any) => el["w14:checkbox"])["w14:checkbox"];
            const uncheckedElement = uncheckedSdtPr.find((el: any) => el["w14:checkbox"])["w14:checkbox"];

            expect(checkedElement.find((el: any) => el["w14:checked"])["w14:checked"]._attr["w14:val"]).to.equal("1");
            expect(uncheckedElement.find((el: any) => el["w14:checked"])["w14:checked"]._attr["w14:val"]).to.equal("0");
        });
        ...
        it("should generate correct symbol configurations", () => {
            ...
            const checkedState = checkboxContent.find((el: any) => el["w14:checkedState"]);
            const checkedStateElements = checkedState["w14:checkedState"];
            const checkedFont = checkedStateElements.find((el: any) => el["w14:font"]);
            const checkedVal = checkedStateElements.find((el: any) => el["w14:val"]);
            expect(checkedVal["w14:val"]._attr["w14:val"]).to.equal("☑");
            expect(checkedFont["w14:font"]._attr["w14:val"]).to.equal("Wingdings");

            const uncheckedState = checkboxContent.find((el: any) => el["w14:uncheckedState"]);
            const uncheckedStateElements = uncheckedState["w14:uncheckedState"];
            const uncheckedFont = uncheckedStateElements.find((el: any) => el["w14:font"]);
            const uncheckedVal = uncheckedStateElements.find((el: any) => el["w14:val"]);
            expect(uncheckedVal["w14:val"]._attr["w14:val"]).to.equal("☐");
            expect(uncheckedFont["w14:font"]._attr["w14:val"]).to.equal("Arial Unicode MS");
        });
        ...
        it("should handle all enhanced properties", () => {
            ...
            expect(sdtPr.find((el: any) => el["w14:checkbox"])).to.exist;
        });
    ...
    describe("XML Structure Validation - Critical Namespace Tests", () => {
        ...
        it("should generate correct w14:checkbox namespace structure (regression test)", () => {
            ...
            const checkboxElement = sdtPr.find((prop: any) => prop["w14:checkbox"]);
            expect(checkboxElement).to.exist;
            ...
            const checkedProp = checkboxProps.find((prop: any) => prop["w14:checked"]);
            ...
            const checkedState = checkboxProps.find((prop: any) => prop["w14:checkedState"]);
            ...
            const fontProp = checkedStateProps.find((prop: any) => prop["w14:font"]);
            const valProp = checkedStateProps.find((prop: any) => prop["w14:val"]);
        ...
        it("should generate correct unchecked state with w14: namespace", () => {
            ...
            const sdtPr = xmlTree["w:sdt"][0]["w:sdtPr"];
            const checkboxElement = sdtPr.find((prop: any) => prop["w14:checkbox"]);
            const checkedProp = checkboxElement["w14:checkbox"].find((prop: any) => prop["w14:checked"]);
        ...
        it("should match existing CheckBox XML structure pattern", () => {
            ...
            const checked = checkboxProps.find((p: any) => p["w14:checked"]);
            const checkedState = checkboxProps.find((p: any) => p["w14:checkedState"]);
            const uncheckedState = checkboxProps.find((p: any) => p["w14:uncheckedState"]);
        ...
        it("should handle custom symbols in XML correctly", () => {
            ...
            const checkedFont = checkedState["w14:checkedState"].find((p: any) => p["w14:font"]);
            const uncheckedFont = uncheckedState["w14:uncheckedState"].find((p: any) => p["w14:font"]);
            ...
            const checkedChar = checkedState["w14:checkedState"].find((p: any) => p["w14:val"]);
            const uncheckedChar = uncheckedState["w14:uncheckedState"].find((p: any) => p["w14:val"]);
        ...
        it("should maintain checkbox XML structure integrity under nesting", () => {
            ...
            const xmlTree = formatter.format(nestedCheckbox, mockContext);
            const xmlString = JSON.stringify(xmlTree);
