...
const mockContext = { stack: [] } as any;
...
describe("RunContentControl", () => {
    ...
    describe("#constructor()", () => {
        ...
        it("should warn about spaces in tag", () => {
            const consoleSpy = vi.spyOn(console, "warn").mockImplementation(() => {});

            new RunContentControl({
                tag: "Customer Name", // Tag with spaces should trigger warning
                children: [new TextRun("Test")],
            });

            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("Tag 'Customer Name' contains spaces"));
    ...
    describe("#prepForXml()", () => {
        ...
        it("should generate unique numeric ID", () => {
            ...
            const idElement1 = sdtPr1.find((el: any) => el["w:id"]);
            const idElement2 = sdtPr2.find((el: any) => el["w:id"]);
    ...
    describe("Nested Content Controls", () => {
        ...
        it("should generate correct OOXML structure for nested run controls", () => {
            ...
            const nestedTagElement = nestedSdtPr.find((el: any) => el["w:tag"]);
        ...
        it("should maintain unique IDs for all nested run controls", () => {
            ...
            const outerIdElement = outerSdtPr.find((el: any) => el["w:id"]);
            ...
            const innerIdElement = innerSdtPr.find((el: any) => el["w:id"]);
    ...
    describe("Enhanced Properties", () => {
        it("should handle appearance property", () => {
            ...
            const appearanceElement = sdtPr.find((el: any) => el["w:appearance"]);
            expect(appearanceElement).to.exist;
            expect(appearanceElement["w:appearance"]._attr["w:val"]).to.equal("tags");
        });
        ...
        it("should handle color property", () => {
            ...
            const colorElement = sdtPr.find((el: any) => el["w:color"]);
            expect(colorElement).to.exist;
            expect(colorElement["w:color"]._attr["w:val"]).to.equal("FF0000");
        });
        ...
        it("should handle data binding property", () => {
            ...
            const dataBindingElement = sdtPr.find((el: any) => el["w:dataBinding"]);
            expect(dataBindingElement).to.exist;
            expect(dataBindingElement["w:dataBinding"]._attr["w:xpath"]).to.equal("/root/customer/name");
            expect(dataBindingElement["w:dataBinding"]._attr["w:storeItemID"]).to.equal("{12345678-1234-1234-1234-123456789012}");
        });
        ...
        it("should use default values for optional enhanced properties", () => {
            ...
            expect(sdtPr.find((el: any) => el["w:appearance"])).to.not.exist;
            expect(sdtPr.find((el: any) => el["w:color"])).to.not.exist;
            expect(sdtPr.find((el: any) => el["w:dataBinding"])).to.not.exist;
        ...
        it("should handle lock properties", () => {
            ...
            const lockElement = sdtPr.find((el: any) => el["w:lock"]);
        ...
        it("should handle partial lock properties", () => {
            ...
            expect(lockElement).to.exist;
            expect(lockElement["w:lock"]._attr["w:contentLocked"]).to.equal("1");
            expect(lockElement["w:lock"]._attr).to.not.have.property("w:sdtLocked");
        });
        ...
        it("should handle placeholder property", () => {
            ...
            const placeholderElement = sdtPr.find((el: any) => el["w:showingPlcHdr"]);
        ...
        it("should handle multiLine text property", () => {
            ...
            expect(textElement["w:text"]["w:multiLine"]._attr["w:val"]).to.equal("1");
        });
        ...
        it("should handle defaultStyle properties", () => {
            ...
            const rPrContent = rPrElement["w:rPr"];
            expect(rPrContent.find((el: any) => el["w:b"])).to.exist;
            expect(rPrContent.find((el: any) => el["w:i"])).to.exist;
            expect(rPrContent.find((el: any) => el["w:color"])["w:color"]._attr["w:val"]).to.equal("FF0000");
            expect(rPrContent.find((el: any) => el["w:sz"])["w:sz"]._attr["w:val"]).to.equal("28");
            expect(rPrContent.find((el: any) => el["w:rFonts"])["w:rFonts"]._attr["w:ascii"]).to.equal("Arial");
        });
    ...
    describe("XML Structure Validation", () => {
        ...
        it("should generate correct OOXML structure for basic run control", () => {
            ...
            const xmlTree = formatter.format(control, mockContext);
            ...
            const hasAlias = sdtPr.some((prop: any) => prop["w:alias"] && prop["w:alias"]._attr["w:val"] === "Test Title");
            const hasTag = sdtPr.some((prop: any) => prop["w:tag"] && prop["w:tag"]._attr["w:val"] === "TestTag");
            const hasId = sdtPr.some((prop: any) => prop["w:id"] && prop["w:id"]._attr["w:val"]);
            const hasText = sdtPr.some((prop: any) => prop["w:text"]);
        ...
        it("should generate correct XML for nested run controls", () => {
            ...
            const xmlTree = formatter.format(outerControl, mockContext);
        ...
        it("should maintain unique numeric IDs across multiple instances", () => {
            ...
            const xml1 = formatter.format(control1, mockContext);
            const xml2 = formatter.format(control2, mockContext);

            // Extract IDs from XML
            const id1 = xml1["w:sdt"][0]["w:sdtPr"].find((prop: any) => prop["w:id"])["w:id"]._attr["w:val"];
            const id2 = xml2["w:sdt"][0]["w:sdtPr"].find((prop: any) => prop["w:id"])["w:id"]._attr["w:val"];

            // IDs should be different numeric values
            expect(id1).to.be.a("string");
            expect(id2).to.be.a("string");
            expect(id1).not.to.equal(id2);
            expect(parseInt(id1)).to.be.a("number");
            expect(parseInt(id2)).to.be.a("number");
        });
    ...
    describe("CRITICAL: richText Parameter Tests", () => {
        it("should generate w:text for richText: false (default)", () => {
            ...
            const richTextElement = sdtPr.find((el: any) => el["w:richText"]);
        ...
        it("should maintain richText property through multiple controls", () => {
            ...
            expect(plainSdtPr.find((el: any) => el["w:text"])).to.exist;
            expect(plainSdtPr.find((el: any) => el["w:richText"])).to.not.exist;

            // Rich control should have w:richText
            expect(richSdtPr.find((el: any) => el["w:richText"])).to.exist;
            expect(richSdtPr.find((el: any) => el["w:text"])).to.not.exist;
        });
    });
    ...
    describe("CRITICAL: OOXML Compliance Tests", () => {
        ...
        it("should generate OOXML that matches Word's expected SDT structure", () => {
            ...
            const hasTag = propsArray.some((prop: any) => prop["w:tag"]);
            const hasId = propsArray.some((prop: any) => prop["w:id"]);
            const hasTextOrRich = propsArray.some((prop: any) => prop["w:text"] || prop["w:richText"]);
