...
export class BlockContentControl extends FileChild {
    ...
    private validateOptions(options: IBlockContentControlOptions): void {
        ...
        if (options.tag.includes(" ")) {
            console.warn(
                `BlockContentControl: Tag '${options.tag}' contains spaces, which may cause issues in some scenarios. ` +
                    "Consider using camelCase or underscores instead (e.g., 'CustomerAddress' or 'customer_address').",
            );
        }
        ...
        if (options.title && options.title.length > 255) {
            console.warn(
                `BlockContentControl: Title '${options.title.substring(0, 50)}...' is longer than 255 characters. ` +
