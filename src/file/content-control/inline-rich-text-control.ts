...
export class InlineRichTextContentControl extends XmlComponent {
    ...
    private validateOptions(options: IRunContentControlOptions): void {
        ...
        // Warn about spaces in tag (common issue)
        if (options.tag.includes(" ")) {
            console.warn(
                `InlineRichTextContentControl: Tag '${options.tag}' contains spaces. ` +
                    `While not strictly forbidden, consider using camelCase or underscores ` +
                    `for better programmatic access (e.g., 'customerName' or 'customer_name').`,
            );
        }
    }
